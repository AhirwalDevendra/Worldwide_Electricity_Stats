...
if User_Menu == 'Worldwide' and Year == 'Overall':
    ...
    fig = px.line(Electricity_generation_YearWise, x='year', y='electricity_generation',
                  labels=({'year': 'Year', 'electricity_generation': 'Electricity Generation in (Twh)'}),
    ...
    Worldwide_fig4 = px.bar(Worldwiswe_Renewable, x='Electricity', y='Percentage', color='Electricity',
                            text='Percentage',
    ...
    Worldwide_fig6 = px.line(Electricity_by_other, x='year',
                             ...
                             labels=({'year': 'Year', 'value': 'Electricity in (Twh)'}),
                             title='Electricity Generation Over The Year')
    # A1.8) -->
    ...
    Wordwide_fig8 = px.line(Demand_vs_supply, x='year', y=['electricity_generation', 'electricity_demand'],
                            markers='year', labels=({'year': 'Year', 'value': 'Electricity in Twh'}),
                            title='Electricity Generation vs Demand Curve')

    # 9)
    Worldwide_Top = df.groupby('country')['electricity_generation'].sum().sort_values(ascending=False).head(
        10).reset_index()
    Worldwide_fig11 = px.bar(Worldwide_Top, x='country', y='electricity_generation', color='country', labels=(
    {'country': 'Country', 'electricity_generation': 'Electricity Generation Value in Twh'}),
                             title='Top 10 Most Electricity Generation Country From 2000-2022')
...
if User_Menu == 'Worldwide' and Year != 'Overall':
    ...
    st.markdown("""***""")
    st.subheader('Electricity That Mostly Produced From --')
    x1 = df[df['year'] == Year].groupby('country')['coal_electricity'].sum().sort_values(ascending=False).head(1).index[
        0]
    x2 = df[df['year'] == Year].groupby('country')['gas_electricity'].sum().sort_values(ascending=False).head(1).index[
        0]
    x3 = df[df['year'] == Year].groupby('country')['oil_electricity'].sum().sort_values(ascending=False).head(1).index[
        0]
    x4 = \
    df[df['year'] == Year].groupby('country')['nuclear_electricity'].sum().sort_values(ascending=False).head(1).index[0]

    y1 = \
    df[df['year'] == Year].groupby('country')['hydro_electricity'].sum().sort_values(ascending=False).head(1).index[0]
    y2 = \
    df[df['year'] == Year].groupby('country')['solar_electricity'].sum().sort_values(ascending=False).head(1).index[0]
    y3 = df[df['year'] == Year].groupby('country')['wind_electricity'].sum().sort_values(ascending=False).head(1).index[
        0]
    y4 = \
    df[df['year'] == Year].groupby('country')['biofuel_electricity'].sum().sort_values(ascending=False).head(1).index[0]
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.info('Coal Electricity')
        st.subheader(x1)
    ...
    x = round(df[df['year'] == Year]['electricity_generation'].sum() * 1000000000 / df[df['year'] == Year][
        'population'].sum())
    ...
    Electrity_by_year = df[df['year'] == Year][
        ['coal_electricity', 'gas_electricity', 'oil_electricity', 'nuclear_electricity', 'hydro_electricity',
         'solar_electricity', 'biofuel_electricity', 'wind_electricity',
         'other_renewable_exc_biofuel_electricity']].sum().reset_index()
    fig1 = px.bar(Electrity_by_year, x='index', y=0, labels=({'index': 'Electricity Mode', '0': 'Value in Twh'}),
                  color='index', title=f'Electricity Production by Year {Year}  ')
    ...
    fig2 = px.pie(Country_wise, values='Percentage', color='Electricity',
                  title='Renewable Vs Non_Renewable Electricity Share', hole=0.4)
    # B1.3) -->
    ...
    # 6)
    Worldwide_Top = df[df['year'] == Year].groupby('country')['electricity_generation'].sum().sort_values(
        ascending=False).head(10).reset_index()
    Worldwide_fig11 = px.bar(Worldwide_Top, x='country', y='electricity_generation', color='country', labels=(
    {'country': 'Country', 'electricity_generation': 'Electricity Generation Value in Twh'}),
                             title='Top 10 Most Electricity Generation Country')
...
if User_Menu == 'Continent Wise' and Year == 'Overall' and Continent != 'Overall':
    ...
    st.subheader('Electricity That Mostly Produced From --')
    x1 = \
    df[df['Continent'] == Continent].groupby('country')['coal_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]
    x2 = df[df['Continent'] == Continent].groupby('country')['gas_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]
    x3 = df[df['Continent'] == Continent].groupby('country')['oil_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]
    x4 = \
    df[df['Continent'] == Continent].groupby('country')['nuclear_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]

    y1 = \
    df[df['Continent'] == Continent].groupby('country')['hydro_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]
    y2 = \
    df[df['Continent'] == Continent].groupby('country')['solar_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]
    y3 = \
    df[df['Continent'] == Continent].groupby('country')['wind_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]
    y4 = \
    df[df['Continent'] == Continent].groupby('country')['biofuel_electricity'].sum().sort_values(ascending=False).head(
        1).index[0]
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.info('Coal Electricity')
    ...
    # 2 --->
    Electricity_per_capita_ContinentWise = \
    df[df['Continent'] == Continent][['year', 'electricity_generation', 'population']].groupby('year')[
        ['electricity_generation', 'population']].sum().reset_index()
    Electricity_per_capita_ContinentWise['Electricity_per_capita'] = (
                Electricity_per_capita_ContinentWise['electricity_generation'] * 1000000000 /
                Electricity_per_capita_ContinentWise['population'])
    Continent_fig5 = px.line(Electricity_per_capita_ContinentWise, x='year', y='Electricity_per_capita', markers='year',
                             labels=({'year': 'Year', 'Electricity_per_capita': 'Electricity Per Capita (KWH)'}),
                             title='Electricity Per Capita ')

    # 3-->
    ContinentWise = df[df['Continent'] == Continent][
    ...
    # 4--->
    Electricity_by_other_ContinentWise = df[df['Continent'] == Continent].groupby('year')[
        ['coal_electricity', 'gas_electricity', 'oil_electricity', 'nuclear_electricity', 'hydro_electricity',
         'solar_electricity', 'biofuel_electricity', 'wind_electricity',
         'other_renewable_exc_biofuel_electricity']].sum().reset_index()
    Continent_fig6 = px.line(Electricity_by_other_ContinentWise, x='year',
                             y=['coal_electricity', 'gas_electricity', 'oil_electricity', 'nuclear_electricity',
                                'hydro_electricity', 'solar_electricity', 'biofuel_electricity', 'wind_electricity',
                                'other_renewable_exc_biofuel_electricity'],
                             labels=({'year': 'Year', 'value': 'Electricity in (Twh)'}),
                             title='Electricity Generation Mode Over the Year')
    ...
    Continent_fig3 = px.bar(ContinentWise_Non_Renewable, x='Electricity', y='Percentage', color='Electricity',
                            text='Percentage', title='Non Renewable Electricity Share ')
    # 6 -->
    ContinentWise_Renewable = df[df['Continent'] == Continent][
        ['hydro_electricity', 'solar_electricity', 'biofuel_electricity', 'wind_electricity',
         'other_renewable_exc_biofuel_electricity']].sum().reset_index()
    ...
    # 7) -->
    ContinentWise_fossile = df[df['Continent'] == Continent][
        ['fossile_electricity', 'Non_fossile_electricity']].sum().reset_index()
    ContinentWise_fossile.rename(columns={'index': 'Electricity', 0: 'Production in Twh'}, inplace=True)
    ContinentWise_fig7 = px.pie(ContinentWise_fossile, values='Production in Twh', names='Electricity',
                                title='Fossil Vs Non-Fossil Share', hole=0.4)

    # 8) -->
    Demand_vs_supply = df[df['Continent'] == Continent].groupby('year')[
    ...
    # 9) -->
    Continent_Top = df[df['Continent'] == Continent].groupby('country')['electricity_generation'].sum().sort_values(
        ascending=False).head(10).reset_index()
    Continent_fig11 = px.bar(Continent_Top, x='country', y='electricity_generation', color='country', labels=(
    {'country': 'Country', 'electricity_generation': 'Electricity Generation Value in Twh'}),
                             title='Top 10 Most Electricity Generation Country From 2000-2022')
    ...
    st.plotly_chart(Continent_fig11)
...
if User_Menu == 'Continent Wise' and Year != 'Overall' and Continent != 'Overall':
    ...
    st.subheader('Electricity That Mostly Produced From --')
    x1 = \
    df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')['coal_electricity'].sum().sort_values(
        ascending=False).head(1).index[0]
    x2 = \
    df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')['gas_electricity'].sum().sort_values(
        ascending=False).head(1).index[0]
    x3 = \
    df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')['oil_electricity'].sum().sort_values(
        ascending=False).head(1).index[0]
    x4 = df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')[
        'nuclear_electricity'].sum().sort_values(ascending=False).head(1).index[0]

    y1 = \
    df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')['hydro_electricity'].sum().sort_values(
        ascending=False).head(1).index[0]
    y2 = \
    df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')['solar_electricity'].sum().sort_values(
        ascending=False).head(1).index[0]
    y3 = \
    df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')['wind_electricity'].sum().sort_values(
        ascending=False).head(1).index[0]
    y4 = df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')[
        'biofuel_electricity'].sum().sort_values(ascending=False).head(1).index[0]
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.info('Coal Electricity')
        st.subheader(x1)
    ...
    x = round(df[(df['year'] == Year) & (df['Continent'] == Continent)]['electricity_generation'].sum() * 1000000000 /
              df[(df['year'] == Year) & (df['Continent'] == Continent)]['population'].sum())
    ...
    # 6)->
    Continent_Top = df[(df['year'] == Year) & (df['Continent'] == Continent)].groupby('country')[
        'electricity_generation'].sum().sort_values(ascending=False).head(10).reset_index()
    Continent_fig11 = px.bar(Continent_Top, x='country', y='electricity_generation', color='country', labels=(
    {'country': 'Country', 'electricity_generation': 'Electricity Generation Value in Twh'}),
                             title='Top 10 Most Electricity Generation Country')
...
if User_Menu == 'Country Wise' and Year == 'Overall' and Country != 'Overall':
    ...
    CountryWise = df[df['country'] == Country][
        ['non_renewable_electricity', 'renewable_electricity']].sum().reset_index()
    CountryWise.rename(columns={'index': 'Electricity', 0: 'Production in Twh'}, inplace=True)
    Country_fig2 = px.pie(CountryWise, values='Production in Twh', names='Electricity',
                          title='Renewable Vs Non-Renewable Share', hole=0.4)
...
if User_Menu == 'Country Wise' and Year != 'Overall' and Country != 'Overall':
    ...
    x = round(df[(df['year'] == Year) & (df['country'] == Country)]['electricity_generation'].sum() * 1000000000 /
              df[(df['year'] == Year) & (df['country'] == Country)]['population'].sum())
